...
export function IssuerHome({
  ...
  return (
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(19rem,0.65fr)] xl:gap-7">
      <section
        aria-labelledby="issuer-home-title"
        className="rounded-[1.25rem] border border-brand-700 bg-brand-900 p-6 text-white shadow-sm sm:p-7 lg:p-8"
      >
        <p className="text-sm font-semibold text-teal-100">Portal del emisor</p>
        <h1
          id="issuer-home-title"
          className="mt-3 max-w-3xl text-3xl leading-tight font-bold tracking-tight sm:text-4xl"
        >
          {'Gestion\u00e1 las credenciales de '}
          {membership.issuerName}
        </h1>
        <p className="mt-4 max-w-2xl leading-7 text-brand-100/80">
          {'Cre\u00e1, complet\u00e1 y emit\u00ed credenciales desde la instituci\u00f3n activa.'}
        </p>

        <div className="mt-7 grid gap-5 border-t border-white/15 pt-5 lg:grid-cols-[minmax(0,1fr)_minmax(16rem,0.7fr)] lg:items-end">
          <div>
            ...
            <span className="mt-5 flex items-center gap-2 text-sm font-semibold text-teal-100">
          ...
          <div className="flex min-w-0 items-start gap-3 rounded-control border border-white/10 bg-white/5 p-3.5 text-brand-100/80">
      ...
      <aside aria-labelledby="next-capability-title">
        <Card className="overflow-hidden rounded-[1.25rem] border-border-strong bg-surface shadow-sm">
          ...
          <Separator />
          <CardContent className="pt-5 sm:px-7 sm:pb-7">
