"""Wrapper productivo del proveedor para razonamiento contextual — slice F3.5.

Misma disciplina que F3.3B/F3.4, y la clasificacion de fallos HTTP se REUTILIZA en
vez de reimplementarse: tres taxonomias sutilmente distintas para el mismo
proveedor serian una fuente de divergencia silenciosa, y F3.3B.2 ya adjudico la
separacion entre fallo transitorio y fallo determinista.

CONFIGURACION POR ETAPA. Variables propias, no compartidas con las otras dos
etapas: el plan congelado tiene una entrada por etapa justamente para que un
despliegue pueda darles modelos distintos sin migracion.

    CONTEXTUAL_REASONING_OPENAI_API_KEY
    CONTEXTUAL_REASONING_OPENAI_MODEL
    CONTEXTUAL_REASONING_OPENAI_BASE_URL              opcional
    CONTEXTUAL_REASONING_PROVIDER_TIMEOUT_SECONDS     opcional

Un despliegue puede asignar deliberadamente el MISMO valor a las variables de
varias etapas. El runtime no infiere esa relacion ni la aprovecha: cada etapa lee
las suyas.
...
Sin SDK, igual que F3.3B/F3.4: `urllib.request` de la biblioteca estandar, asi que
...
ENV_API_KEY = "CONTEXTUAL_REASONING_OPENAI_API_KEY"
ENV_MODEL = "CONTEXTUAL_REASONING_OPENAI_MODEL"
ENV_BASE_URL = "CONTEXTUAL_REASONING_OPENAI_BASE_URL"
ENV_TIMEOUT = "CONTEXTUAL_REASONING_PROVIDER_TIMEOUT_SECONDS"
...
def configured_model() -> str:
    """El modelo que ESTE despliegue va a solicitar para la etapa 3."""
...
class OpenAIContextualReasoningProvider:
