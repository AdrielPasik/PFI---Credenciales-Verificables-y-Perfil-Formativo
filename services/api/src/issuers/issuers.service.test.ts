...
test('C3a: IssuersService applies the shared institutional rules to course template management', async () => {
  for (const role of [
    IssuerMembershipRole.admin,
    IssuerMembershipRole.operator
  ]) {
    ...
    assert.equal(
      await service.assertUserCanManageCourseTemplatesForIssuer(
        'issuer-user-1',
        'issuer-1'
      ),
      membership
  ...
  for (const membership of forbiddenMemberships) {
    ...
    await assert.rejects(
      service.assertUserCanManageCourseTemplatesForIssuer(
        'issuer-user-1',
        'issuer-arbitrary'
      ),
      ForbiddenException
...
test('IssuersService applies shared institutional rules to text evidence submissions', async () => {
  for (const role of [
    IssuerMembershipRole.admin,
    IssuerMembershipRole.operator
  ]) {
    ...
    assert.equal(
      await service.assertUserCanSubmitTextEvidenceForIssuer(
        'issuer-user-1',
        'issuer-1'
      ),
      membership
  ...
  for (const membership of forbiddenMemberships) {
    ...
    await assert.rejects(
      service.assertUserCanSubmitTextEvidenceForIssuer(
        'issuer-user-1',
        'issuer-arbitrary'
      ),
      ForbiddenException
...
test('IssuersService applies shared institutional rules before issuer-scoped issuance', async () => {
  for (const role of [
    IssuerMembershipRole.admin,
    IssuerMembershipRole.operator
  ]) {
    ...
    assert.equal(
      await service.assertUserCanIssueCredentialForIssuer(
        'issuer-user-1',
        'issuer-1'
      ),
      membership
  ...
  for (const membership of forbiddenMemberships) {
    ...
    await assert.rejects(
      service.assertUserCanIssueCredentialForIssuer(
        'issuer-user-1',
        'issuer-arbitrary'
      ),
      ForbiddenException
