...
export function WalletCredentialDetailView({ detail }: { detail: HolderCredentialDetailVM }) {
  return <section className="grid gap-8" aria-labelledby="holder-credential-title">
    <BackLink />
    ...
    <Card className="overflow-hidden border-brand-700 shadow-sm"><div className={detail.status === 'revoked' ? 'h-1 bg-status-error' : 'h-1 bg-teal-700'} /><CardHeader className="gap-4 sm:p-8"><div className="flex flex-wrap items-center gap-2"><Badge variant={detail.status === 'revoked' ? 'outline' : 'secondary'}>{detail.statusLabel}</Badge><Badge variant="outline">{detail.typeLabel}</Badge></div><div><p className="text-sm font-semibold text-teal-700">Credencial formativa</p><h1 id="holder-credential-title" className="mt-1 text-3xl font-bold tracking-tight text-text-strong">{detail.title}</h1><p className="mt-4 max-w-3xl leading-7 text-text-muted">Esta credencial fue emitida por la institución indicada. Scope permite consultar su integridad y su aporte formativo.</p></div></CardHeader></Card>
