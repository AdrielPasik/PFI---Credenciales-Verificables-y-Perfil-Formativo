...
from .b24_versions import B24_VERSIONS
...
def build_b24_objective_analysis(
    proposal: dict[str, Any], objective: str
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    """Deterministic enrichment of the evidence-blind Objective Analysis.

    DELTA_C: `epistemicTarget` is carried through read-only from this stage. It is
    never recomputed downstream and `normalizedRequirement` can never modify it.
    """
    ...
    if proposal["decompositionStatus"] == "AMBIGUOUS":
        if proposal["requirements"]:
            results.append(
                validation(
                    "HARD_FACTUAL_INVARIANT",
                    "AMBIGUOUS_DECOMPOSITION_MUST_NOT_SELECT_REQUIREMENT",
                    "FAIL",
                    "objective",
        ...
        return (
            {
                "schemaVersion": B24_VERSIONS["objective"],
                "originalObjective": objective,
                "objectiveContext": proposal["objectiveContext"],
                "decompositionStatus": "AMBIGUOUS",
                "ambiguityRationale": proposal["ambiguityRationale"],
    ...
    if not proposal["requirements"]:
        results.append(
            validation(
                "HARD_FACTUAL_INVARIANT",
                "RESOLVED_DECOMPOSITION_NEEDS_REQUIREMENT",
                "FAIL",
                "objective",
                "empty",
                affects_epistemic_state=False,
    ...
    for index, item in enumerate(proposal["requirements"], 1):
        req_id = f"req_{index:02d}"
        req_span, status = _span(objective, item["requirementQuote"])
        trace_valid = req_span is not None
        results.append(
            validation(
                "HARD_FACTUAL_INVARIANT",
                "REQUIREMENT_QUOTE_INVALID",
                "PASS" if trace_valid else "FAIL",
                req_id,
                status,
                affects_epistemic_state=not trace_valid,
            )
        )
        ...
        results.append(
            validation(
                "HARD_FACTUAL_INVARIANT",
                "REQUIREMENT_SELECTED_FROM_EVALUABLE_SEGMENT",
                "PASS" if selected else "FAIL",
                req_id,
                item["requirementQuote"],
                affects_epistemic_state=not selected,
            )
        )
        # DELTA_C: the classification itself is a semantic judgment and stays a
        # manual-adjudication dimension. Only its presence/enum is factual.
        results.append(
            validation(
        ...
        for raw in item["qualifiers"]:
            qcount += 1
            scoped, qstatus = _span(
                objective,
                raw["sourcePhrase"],
                start=req_span["charStart"] if req_span else 0,
                end=req_span["charEnd"] if req_span else None,
            )
            if scoped is None and req_span is not None:
                scoped, qstatus = _span(objective, raw["sourcePhrase"])
            valid = scoped is not None
            qid = f"q_{qcount:02d}" if raw["role"] == "MATERIAL_QUALIFIER" else None
            overlap = raw["role"] == "MATERIAL_QUALIFIER" and raw["sourcePhrase"] in context_or_wrapper
            results.append(
                validation(
                    "HARD_FACTUAL_INVARIANT",
                    "QUALIFIER_SCOPED_TRACE_VALID",
                    "PASS" if valid else "FAIL",
                    qid or f"trace_q_{qcount:02d}",
                    qstatus,
                    affects_epistemic_state=not valid,
                )
            )
            results.append(
                validation(
                    "HARD_FACTUAL_INVARIANT",
                    "SEGMENTED_CONTEXT_WRAPPER_NOT_MATERIAL_QUALIFIER",
                    "PASS" if not overlap else "FAIL",
                    qid or f"trace_q_{qcount:02d}",
                    raw["sourcePhrase"],
                    affects_epistemic_state=overlap,
                )
            )
        ...
        requirements.append(
            {
                ...
                "epistemicTarget": item["epistemicTarget"],
                "epistemicTargetRationale": item["epistemicTargetRationale"],
                ...
                "materialQualifiers": [q for q in qualifiers if q["role"] == "MATERIAL_QUALIFIER"],
                "contextAnnotations": [q for q in qualifiers if q["role"] == "CONTEXTUAL"],
                "structuralWrappers": [q for q in qualifiers if q["role"] == "STRUCTURAL_WRAPPER"],
            }
        )
...
def source_observability_facts(
    snapshots: list[dict[str, Any]], evidence_units: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    observed: dict[str, list[str]] = defaultdict(list)
    for unit in evidence_units:
        observed[unit["sourceTrace"]["sourceId"]].append(unit["evidenceUnitId"])
    return [
        {
            "sourceId": snapshot["source"]["sourceId"],
            "coverageStatus": snapshot["coverageStatus"],
            "observedEvidenceUnitIds": observed[snapshot["source"]["sourceId"]],
            "extractionDiagnostics": snapshot.get("diagnostics", []),
        }
