...
function WalletHomeContent() {
  ...
  return <WalletHomeView profileState={profileState} credentialsState={credentialsState} showProfileShare />;
}

export function WalletHomeView({ profileState, credentialsState, showProfileShare = false }: { profileState: HolderProfileLoadState; credentialsState: HolderCredentialsLoadState; showProfileShare?: boolean }) {
  ...
  return <div className="grid gap-8"><header><p className="text-sm font-semibold text-teal-700">Espacio personal</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-text-strong sm:text-4xl">Mi perfil formativo</h1><p className="mt-3 max-w-2xl leading-7 text-text-muted">Una vista de tu trayectoria construida a partir de tus credenciales formativas y de los análisis disponibles en Traza.</p></header>{profileState.status === 'loading' ? <LoadingState label="Cargando tu perfil formativo" /> : null}{profileState.status === 'ready' ? <>{showProfileShare ? <ProfileShareAction /> : null}<HolderProfilePanel profile={profileState.profile} /></> : null}{profileState.status === 'empty' ? <HolderProfileEmptyPanel /> : null}{profileState.status === 'error' ? <FeedbackAlert variant="warning" title="No pudimos cargar tu perfil formativo">Tus credenciales siguen disponibles. Podés volver a intentar más tarde.</FeedbackAlert> : null}<section aria-labelledby="wallet-credentials-title" className="grid gap-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-semibold text-teal-700">Tu biblioteca</p><h2 id="wallet-credentials-title" className="mt-1 text-2xl font-bold tracking-tight text-text-strong">Tus credenciales</h2><p className="mt-2 text-sm leading-6 text-text-muted">Consultá las credenciales formativas disponibles en tu espacio personal.</p></div><Button asChild variant="secondary"><Link href="/wallet/credentials">Ver todas <ArrowRight aria-hidden="true" /></Link></Button></div>{credentialsState.status === 'loading' ? <LoadingState label="Cargando credenciales" /> : null}{credentialsState.status === 'error' ? <FeedbackAlert variant="error" title="No pudimos cargar tus credenciales">{credentialsState.message}</FeedbackAlert> : null}{credentialsState.status === 'ready' && credentials.length === 0 ? <EmptyCredentials /> : null}{credentialsState.status === 'ready' && credentials.length > 0 ? <><div className="grid gap-3 sm:grid-cols-3"><SummaryCard label="Emitidas" value={issued.length} /><SummaryCard label="Revocadas" value={revoked.length} /><SummaryCard label="Con análisis disponible" value={credentials.filter((credential) => credential.hasAnalysis).length} /></div><div className="grid gap-4 md:grid-cols-2">{credentials.slice(0, 2).map((credential) => <HolderCredentialCard key={credential.credentialReference} credential={credential} />)}</div></> : null}</section></div>;
}
...
function SummaryCard({ label, value }: { label: string; value: number }) { return <Card><CardContent className="pt-5 sm:pt-6"><p className="text-2xl font-bold text-text-strong">{value}</p><p className="mt-1 text-sm text-text-muted">{label}</p></CardContent></Card>; }
